...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        if (activeService === "flights") {
          const res = await fetch("http://localhost:5001/api/flights/search", {
            method: "POST",
            headers: {
        ...
        const res = await fetch(
          `http://localhost:5001/api/transport/search?${params}`
        );
        const data = await res.json();
  ...
  }, [results, stopsFilter, selectedAirlines, priceRange, activeService]);
  ...
  return (
    <div className="min-h-screen bg-white text-[#1e293b] font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex gap-8">
        ...
        <div className="hidden lg:block w-1/4 h-fit sticky top-32 glass-card rounded-xl p-6">
        ...
        <div className="flex-1">
          ...
          {!loading && (error || results.length === 0) && (
            <div className="glass-card rounded-xl p-10 text-center">
              ...
              <h3 className="text-xl font-bold text-[#0f294d] mb-2">No Flights Found</h3>
